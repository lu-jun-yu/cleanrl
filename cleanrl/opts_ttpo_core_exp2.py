...
def select_next_states(
    ...
    active_tids, roots, tree_e_local, active_counts = [], [], [], []
    env_num_trees = [0] * E
    for e_local, env_idx in enumerate(terminated_envs):
        ...
        for tid in env_tree_ids:
            ...
            active_counts.append(search_count[env_idx].get(tid, 0))
    ...
    if T:
        ...
        search_counts = torch.tensor(active_counts, device=device, dtype=torch.long)
        ...
        for i, tid in enumerate(active_tids):
            max_otrc_scores[terminated_envs[tree_e_local[i]]][tid] = float(max_otrc_score[i].item())

        pool = [v for d in max_otrc_scores for v in d.values()]
        if len(pool) > 1:
            mean_threshold = float(np.mean(pool))
            eligible = (max_otrc_score > mean_threshold) & (search_counts < max_search)
