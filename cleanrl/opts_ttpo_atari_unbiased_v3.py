# Unbiased value-degradation OPTS-TTPO for Atari
...
from opts_ttpo_core_unbiased_v3 import compute_branch_weight, compute_tree_gae, select_next_states
...
class Args:
    ...
    resume_path: str = ""
    """checkpoint path to resume from; defaults to checkpoint_dir/exp_name/env_id/seed{seed}.pt"""
...
def get_checkpoint_path(args):
    return os.path.join(args.checkpoint_dir, args.exp_name, args.env_id, f"seed{args.seed}.pt")
...
if __name__ == "__main__":
    ...
    search_advantages = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(start_iteration, args.num_iterations + 1):
        ...
        # Latest M_{i,T} for every complete tree across all environments.
        terminal_estimates = [{} for _ in range(args.num_envs)]
